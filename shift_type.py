...
class ShiftType(Document):
    ...
    def process_auto_attendance(self):
        ...
        for key, group in itertools.groupby(
            logs, key=lambda x: (x["employee"], x["shift_start"])
        ):
        ...
        for employee in self.get_assigned_employees(
    ...
    def get_assigned_employees(self, from_date=None, consider_default_shift=False):
        ...
        assigned_employees = frappe.get_all(
            "Shift Assignment", filters=filters, pluck="employee"
        )
        ...
        inactive_employees = frappe.db.get_all(
            "Employee", {"status": "Inactive"}, pluck="name"
        )
    ...
    def get_holiday_list(self, employee: str) -> str:
        holiday_list_name = self.holiday_list or get_holiday_list_for_employee(
            employee, False
        )
